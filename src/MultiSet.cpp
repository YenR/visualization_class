...
const void MultiSet::switchElements(const int e1, const int e2) {
	std::swap(m_DataElements[e1].name, m_DataElements[e2].name);
	m_DataElements[e1].values.swap(m_DataElements[e2].values);
	std::swap(m_Variates[e1].name, m_Variates[e2].name);
	std::swap(m_Variates[e1].min, m_Variates[e2].min);
	std::swap(m_Variates[e1].max, m_Variates[e2].max);
